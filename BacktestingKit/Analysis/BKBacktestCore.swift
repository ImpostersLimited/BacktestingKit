...
private func updatePosition(_ position: inout BKPosition, _ bar: BKBar) {
...
private func finalizePosition(_ position: BKPosition, exitTime: Date, exitPrice: Double, exitReason: String) -> BKTrade {
...
public func backtest(
    strategy: BKStrategy,
    inputSeries: [BKBar],
    options: BKBacktestOptions = BKBacktestOptions()
) -> (trades: [BKTrade], lastStatus: PositionStatus) {
    ...
    var completedTrades: [BKTrade] = []
    var positionStatus: PositionStatus = .none
    var positionDirection: TradeDirection = .long
    var conditionalEntryPrice: Double?
    var openPosition: BKPosition?

    var lookbackBuffer = LookbackBuffer<BKBar>(capacity: lookbackPeriod)

    func enterPosition(_ options: BKEnterPositionOptions?) {
    ...
    func closePosition(_ bar: BKBar, _ exitPrice: Double, _ exitReason: String) {
    ...
    for bar in indicatorsSeries {
        ...
        case .none:
            strategy.entryRule(enterPosition, BKRuleParams(bar: bar, lookback: lookback, parameters: strategyParameters))
        case .enter:
            if let conditionalEntryPrice {
                if positionDirection == .long {
                    if bar.high < conditionalEntryPrice { break }
                } else {
                    if bar.low > conditionalEntryPrice { break }
                }
            }

            let entryPrice = bar.open
            var newPosition = BKPosition(
            ...
            if let stopLoss = strategy.stopLoss {
                let stopDistance = stopLoss(BKOpenPositionRuleArgs(
                    entryPrice: entryPrice,
            ...
            if let trailingStopLoss = strategy.trailingStopLoss {
                let trailingStopDistance = trailingStopLoss(BKOpenPositionRuleArgs(
                    entryPrice: entryPrice,
                    position: newPosition,
                    bar: bar,
                    lookback: lookback,
                ...
                let trailingStopPrice = newPosition.direction == .long
                    ...
                    : entryPrice + trailingStopDistance
                if let initialStopPrice = newPosition.initialStopPrice {
                    if newPosition.direction == .long {
                        newPosition.initialStopPrice = max(initialStopPrice, trailingStopPrice)
                    } else {
                        newPosition.initialStopPrice = min(initialStopPrice, trailingStopPrice)
                    }
                } else {
                ...
                if options.recordStopPrice {
                    newPosition.stopPriceSeries = [
                        BKTimestampedValue(time: bar.time, value: newPosition.curStopPrice ?? 0)
                    ]
                }
            }
            ...
            if let curStop = newPosition.curStopPrice {
                ...
                if options.recordRisk {
                    newPosition.riskSeries = [
                        BKTimestampedValue(time: bar.time, value: newPosition.curRiskPct ?? 0)
                    ]
                }
            }

            if let profitTarget = strategy.profitTarget {
                let profitDistance = profitTarget(BKOpenPositionRuleArgs(
                    entryPrice: entryPrice,
                    position: newPosition,
                    bar: bar,
                    lookback: lookback,
        ...
        case .position:
            ...
            if let trailingStopLoss = strategy.trailingStopLoss {
                ...
                if options.recordStopPrice {
                    let nextValue = BKTimestampedValue(time: bar.time, value: working.curStopPrice ?? 0)
                    if var stopPriceSeries = working.stopPriceSeries {
                        stopPriceSeries.append(nextValue)
                        working.stopPriceSeries = stopPriceSeries
                    } else {
                        working.stopPriceSeries = [nextValue]
                    }
                }
            }
            ...
            if let curRisk = working.curRiskPct, options.recordRisk {
                let nextValue = BKTimestampedValue(time: bar.time, value: curRisk)
                if var riskSeries = working.riskSeries {
                    riskSeries.append(nextValue)
                    working.riskSeries = riskSeries
                } else {
                    working.riskSeries = [nextValue]
                }
            }

            if let exitRule = strategy.exitRule {
                exitRule(exitPosition, BKOpenPositionRuleArgs(
                    entryPrice: working.entryPrice,
                    position: working,
                    bar: bar,
                    lookback: lookback,
...
/// Executes `analyze`.
public func analyze(startingCapital: Double, trades: [BKTrade]) -> BKAnalysis {
