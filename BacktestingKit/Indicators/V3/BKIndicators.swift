...
/// Executes `v3setTechnicalIndicators`.
...
private func v3bakeIndicators(
    _ inputSeries: DFDataFrame<Int, BKBar>,
    rule: BKV3_SimulationRule,
    seriesCache: inout [String: DFSeries<Int, Double>]
) -> (DFDataFrame<Int, BKBar>, Int) {
...
private func extractIndicatorSpec(_ rule: BKV3_SimulationRule, isOne: Bool) -> IndicatorSpec? {
    if isOne {
...
private func applyIndicatorDF(
    dataFrame: inout DFDataFrame<Int, BKBar>,
    spec: IndicatorSpec,
    seriesCache: inout [String: DFSeries<Int, Double>]
) -> Int {
...
private func assignSeries(_ series: inout [BKBar], name: String, values: [Double?]) {
...
private func ema(_ values: [Double], period: Int) -> [Double?] {
    ...
    for i in period..<values.count {
        let current = (values[i] - previous) * k + previous
        result[i] = current
        previous = current
    }
    return result
...
private func rsi(_ values: [Double], period: Int) -> [Double?] {
    ...
    for i in period..<values.count {
        ...
        if averageLoss < Double.ulpOfOne {
            result[i] = averageGain < Double.ulpOfOne ? 50 : 100
            continue
        }
...
private func macd(_ values: [Double], fast: Int, slow: Int, signal: Int) -> (macd: [Double?], signal: [Double?], histogram: [Double?]) {
    ...
    if signal > 0, startIndex < values.count {
        ...
        if macdValues.count >= signal {
            let signalStart = startIndex + signal - 1
            let seed = Array(macdValues.prefix(signal)).reduce(0, +) / Double(signal)
            signalLine[signalStart] = seed
            if macdValues.count > signal {
                let k = 2.0 / Double(signal + 1)
                var previous = seed
                for offset in signal..<macdValues.count {
                    let current = (macdValues[offset] - previous) * k + previous
                    let idx = startIndex + offset
                    if idx < signalLine.count {
                        signalLine[idx] = current
                    }
                    previous = current
                }
            }
...
private func stochasticFast(series: [BKBar], periodK: Int, periodD: Int) -> (percentK: [Double?], percentD: [Double?]) {
    let count = series.count
...
private func stochasticSlow(series: [BKBar], periodK: Int, periodD: Int, periodSlow: Int) -> (percentK: [Double?], percentD: [Double?]) {
    let fastK = stochasticFast(series: series, periodK: periodK, periodD: periodD).percentK
    let slowK = smaOptional(fastK, period: periodD)
    let slowD = smaOptional(slowK, period: periodSlow)
    return (slowK, slowD)
}
